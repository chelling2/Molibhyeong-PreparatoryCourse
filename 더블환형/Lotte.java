...

{
	...
	public void print(int index, int sa) {

		if (sa == 1) { // 앞출력

			if (index < 0) { // 1 입력시 -1
				
				if(index<0) {  //  그 이하 숫자 입력 시 무조건 첫 데이터 가리키게 하기 위함 
					index=-1;  //  
				}
				Node<E> currentNode = search(index + 1); // 0 찾고
				...
				Node<E> currentNode = search(index+1);  // +1 안해주면 번째 상 그 이전 데이터라 해줘야함 
				Node<E> nextNode = currentNode.next;    // 출력을 현재 데이터로 출력하였기에 
